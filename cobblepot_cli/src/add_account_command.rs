use crate::arguments::{
    account_description, account_name, account_type, parse_account_description, parse_account_name,
    parse_account_type,
};
...
pub fn create_command() -> Command {
    Command::new("add_account").about("Add a new account to your chart of accounts").args([
        account_name(),
        account_description(),
        account_type(),
    ])
...
pub fn command_handler(
    ...
    let account_type = parse_account_type(matches)?;

    let chart_of_accounts = ChartOfAccounts::new(vault_config);
    match chart_of_accounts.create_account(Account::new(
        account_name.clone(),
